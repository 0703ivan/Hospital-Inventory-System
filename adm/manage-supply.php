...
<div class="body-section">
      ...
      <div class="aside-right">
        <h1>Manage</h1>
        <form action="manage-supply.php" method="POST">
          <input type="submit" class="btn btn-info" name="add" value="Add ITEM">
          <input type="submit" class="btn btn-info" name="update" value="Update Info">
          <input type="submit" class="btn btn-info" name="info" value="View Info">
          <input type="submit" class="btn btn-info" name="print" value="Print">
          <input type="submit" class="btn btn-info" name="remove" value="Remove">
        </form>
        <?php
          if(isset($_POST['add'])){
            require('../reg-php/su-regform.php');
          }
          if(isset($_POST['update'])){
            require('../u-php/su-regform.php');
          }
          if(isset($_POST['remove'])){
            require('../d-php/su-regform.php');
          }
        ?>
        <table class="table table-striped table-hover "align="center" cellpadding="10" border="1" cellspacing="0" width="900px">
          <thead>
            <th class="info">ITEM ID</th>
            <th class="success"> ITEM NAME</th>
            <th class="info"> QUANTITY</th>
            <th class="success"> MANUFACTURE DATE</th>
            <th class="info"> EXPIRE DATE</th>
            <th class="success"> SUPPLIER</th>
            <th class="info"> COUNTRY</th>
          </thead>
          <?php 
            require('../connect.php');
            $sql=mysqli_query($con,"SELECT * FROM `supply`") or die("failed to connect");
